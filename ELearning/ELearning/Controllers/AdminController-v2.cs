...
namespace ELearning.Controllers
{
    public class AdminController : Controller
    {
        ...
        private List<StudentTrackingModel> GetAllStudentAssignedStoriesProgress(int studentid)
        {
            ...
            MySqlConnection con = new MySqlConnection(defaultConnection);
            MySqlCommand cmd = con.CreateCommand();
            cmd.CommandText = @"SELECT distinct stud.StoryTitle, stud.grade, stud.totalscore, CAST(((CASE WHEN stud.totalquestions = 0 THEN 0 ELSE ((CAST(stud.grade AS DECIMAL(10,2)))/(CAST((stud.totalquestions * 10) AS DECIMAL(10,2)))) END) * 100.0) AS DECIMAL(10,2)) as ResultPercentage  
                                FROM (
                                    SELECT * FROM (
                                        SELECT s.title as StoryTitle, g.totalquestions, g.grade, (g.totalquestions * 10) as totalscore
                                        FROM stories s   
                                        left join (
                                            select *, ROW_NUMBER() over(partition by sg.storyid order by attempt desc) as row_number from `studentgrades` sg 
                                        ) g on s.id = g.storyid
                                    ...
                                    SELECT * FROM (
                                    	SELECT s.title as StoryTitle, case when sq.totalquestions is null then 0 else sq.totalquestions end as totalquestions
                                            , case when g.grade is null then 0 else g.grade end as grade, case when sq.totalquestions is null then 0 else (sq.totalquestions * 10) end as totalscore 
        ...
        private ResultModel SaveStoryDB(StoryModel model, string action)
        {
            ...
            if (action == "update")
            {
                cmd.CommandText = "UPDATE `stories` SET `title`=@title,`content`=@content,`classid`=@classid,`coverimage`=@coverimage,`updatedby`=@updatedby,`dateupdated`=@dateupdated,`incomplete`=@incomplete WHERE `id` = @id";
                cmd.Parameters.AddWithValue("@id", model.ID);
            ...
            {
                cmd.CommandText = "INSERT INTO `stories`(`title`, `content`, `classid`,`coverimage`,`incomplete`, `addedby`, `dateadded`, `updatedby`, `dateupdated`, `isdeleted`) VALUES (@title,@content,@classid,@coverimage,@incomplete,@addedby,@dateadded,@updatedby,@dateupdated,0);SELECT LAST_INSERT_ID();";
                cmd.Parameters.AddWithValue("@title", model.Title);
                cmd.Parameters.AddWithValue("@content", model.Content);
                cmd.Parameters.AddWithValue("@classid", model.ClassID);
                cmd.Parameters.AddWithValue("@coverimage", model.CoverImageName);
